...
impl ClearViewDailyParser {
    ...
    fn parse_currency(&self, value: &str) -> ParserResult<f64> {
        ...
        cleaned
            .parse::<f64>()
            .map_err(|e| ParserError::TypeConversion {
    ...
    pub fn process(&self) -> ParserResult<PivotTable> {
        ...
        println!(
            "[DEBUG] ClearViewDailyParser processing {} files",
            self.file_paths.len()
        );
        ...
        for row in all_data {
            ...
            // Parse amounts
            let syn_gross =
                self.parse_currency(row.get("Syn Gross Amount").unwrap_or(&"0".to_string()))?;
            let syn_net =
                self.parse_currency(row.get("Syn Net Amount").unwrap_or(&"0".to_string()))?;
