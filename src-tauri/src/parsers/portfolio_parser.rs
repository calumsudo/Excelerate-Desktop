...
impl PortfolioParser {
            ...
            if let Ok(range) = workbook.worksheet_range(sheet_name) {
                match self.extract_merchants_from_sheet(&range, funder_name, db) {
                    ...
                    Err(e) => {
                        ...
                        eprintln!(
                            "Failed to extract merchants from {} sheet: {}",
                            sheet_name, e
                        );
                    }
                }
        ...
        for row_index in (header_row_index + 1)..total_rows {
            ...
            match self.extract_merchant_from_row(range, row_index, &column_indices, funder_name) {
                ...
                Err(e) => {
                    ...
                    eprintln!(
                        "Failed to extract merchant from row {}: {}",
                        row_index + 1,
                        e
                    );
                }
    ...
    fn map_column_indices(&self, headers: &[String]) -> Result<HashMap<String, usize>, String> {
        ...
        let column_mappings = vec![
            (
                "date_funded",
                vec!["Date Funded", "Funded Date", "Fund Date"],
            ),
            (
                "merchant_name",
                vec!["Merchant Name", "Merchant", "Business Name", "DBA"],
            ),
            ("website", vec!["Website", "Web Site", "URL"]),
            (
                "advance_id",
                vec!["Advance ID", "Deal ID", "Advance #", "Deal Number"],
            ),
            (
                "funder_advance_id",
                vec!["Funder Advance ID", "Funder Deal ID", "Funder ID"],
            ),
            (
                "industry",
                vec![
        ...
        for (key, variations) in column_mappings {
            for (idx, header) in headers.iter().enumerate() {
                ...
                for variation in &variations {
                    if header_lower == variation.to_lowercase()
                        || header_lower.contains(&variation.to_lowercase())
                    {
    ...
    fn is_row_empty(
        ...
        if let Some(&merchant_col) = column_indices.get("merchant_name") {
            let merchant_name = range
                .get_value((row_index as u32, merchant_col as u32))
                .and_then(|cell| cell.as_string())
                .unwrap_or_default();
    ...
    fn extract_merchant_from_row(
        ...
        let get_string_value = |key: &str| -> Option<String> {
            column_indices.get(key).and_then(|&col_idx| {
                range
                    .get_value((row_index as u32, col_idx as u32))
                    .and_then(|cell| cell.as_string())
                        ...
                        match cell {
                            ...
                            Data::Float(f) => {
                                ...
                                NaiveDate::from_ymd_opt(1899, 12, 31)
                                    .and_then(|base| {
                                        base.checked_add_signed(Duration::days(adjusted_days))
                                    })
        ...
        let merchant_name =
            get_string_value("merchant_name").ok_or_else(|| "Missing merchant name".to_string())?;
