...
impl BaseParser for BhbParser {
    ...
    fn process_row(&self, row: &HashMap<String, String>) -> ParserResult<Option<ProcessedData>> {
        ...
        let gross_amount = row.get("Participator Gross Amount").ok_or_else(|| {
            ParserError::ProcessingError("Missing Participator Gross Amount".to_string())
        })?;
        ...
        let net_amount = row.get("Net Payment Amount").ok_or_else(|| {
            ParserError::ProcessingError("Missing Net Payment Amount".to_string())
        })?;
