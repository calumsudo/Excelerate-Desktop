...
impl ClearViewPivotProcessor {
    ...
    pub fn process_all_daily_files(&self) -> ParserResult<(PivotTable, String)> {
        ...
        if all_daily_files.is_empty() {
            return Err(ParserError::ProcessingError(
                "No daily files found in folder".to_string(),
            ));
        }
    ...
    pub fn create_daily_aggregated_pivot(
        ...
        if daily_file_paths.is_empty() {
            return Err(ParserError::ProcessingError(
                "No daily files provided".to_string(),
            ));
        }
        ...
        let pivot_path = self.save_pivot_table(&pivot, PivotTableType::DailyAggregated)?;
    ...
    pub fn create_weekly_report_pivot(
        ...
        // Save pivot table to file
        let pivot_path = self.save_pivot_table(&pivot, PivotTableType::WeeklyReport)?;
    ...
    pub fn update_combined_pivot_if_needed(&self) -> ParserResult<Option<(PivotTable, String)>> {
        ...
        self.create_combined_pivot(&daily_pivot, &weekly_pivot)
            .map(Some)
    }
    ...
    fn load_pivot_from_csv(&self, path: &Path) -> ParserResult<PivotTable> {
        ...
        for result in reader.records() {
            ...
            if record.len() >= 5 {
                ...
                let gross = record.get(2).unwrap_or("0").parse::<f64>().unwrap_or(0.0);
                let fee = record.get(3).unwrap_or("0").parse::<f64>().unwrap_or(0.0);
                let net = record.get(4).unwrap_or("0").parse::<f64>().unwrap_or(0.0);
    ...
    pub fn create_combined_pivot(
        ...
        for row in &daily_pivot.rows {
            if row.advance_id != "Totals" {
                let entry = combined_data.entry(row.advance_id.clone()).or_insert((
        ...
        // Save pivot table to file
        let pivot_path = self.save_pivot_table(&combined_pivot, PivotTableType::Combined)?;
    ...
    pub fn get_daily_files_for_week(&self, _start_date: &str) -> Result<Vec<PathBuf>, String> {
        ...
        println!(
            "[DEBUG] Looking for daily files in: {}",
            daily_dir.display()
        );
